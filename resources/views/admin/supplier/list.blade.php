			...
			<div class="row">
				<div class="col-xs-12">
						...
						<form id="frmSearch" method="GET" action="" class="frmSearch" name="frmSearch">
							<div class="panel-body">
								<div class="form-group col-lg-2">
									...
									<div>
										<input type="text" class="form-control input-sm" name="supplier_stitle" @if(isset
										($search['supplier_stitle']) && $search['supplier_stitle'] !='')value="{{$search['supplier_stitle']}}"@endif>
									</div>
								</div>
								<div class="form-group col-lg-2">
									<label class="control-label">Trạng thái</label>
									<div>
										<select name="supplier_status" class="form-control input-sm">
											{!! $optionStatus !!}
										</select>
									</div>
								</div>
					...
					@if(sizeof($data) > 0)
						...
						@endif
						<br>
						<form id="formListItem" method="POST" action="{{FuncLib::getBaseUrl()}}admin/supplier/delete" class="formListItem" name="txtForm">
							<table class="table table-bordered table-hover">
								...
								@foreach($data as $k=>$item)
									<tr>
										...
										<td>
											<label class="pos-rel">
												<input class="ace checkItem" name="checkItem[]" value="{{$item['supplier_id']}}" type="checkbox">
												<span class="lbl"></span>
											</label>
										</td>
										...
										<td>{{date('d/m/Y', $item['supplier_created'])}}</td>
										<td>
											@if($item['supplier_status'] == '1')
										...
										<td>
											<a href="{{FuncLib::getBaseUrl()}}admin/supplier/edit/{{$item['supplier_id']}}" title="Cập nhật">
