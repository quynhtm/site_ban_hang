            ...
            <div class="row">
                <div class="col-xs-12">
                        ...
                        <form id="frmSearch" method="GET" action="" class="frmSearch" name="frmSearch">
                            <div class="panel-body">
                                <div class="form-group col-sm-2">
                                    ...
                                    <div>
                                        <input type="text" class="form-control input-sm" name="member_name" @if(isset($search['member_name']) && $search['member_name'] !='')value="{{$search['member_name']}}"@endif>
                                    </div>
                                </div>
                            ...
                            <div class="panel-footer text-right">
                                <a class="btn btn-danger btn-sm" href="{{FuncLib::getBaseUrl()}}admin/member/edit"><i class="ace-icon fa fa-plus-circle"></i>Thêm mới</a>
                    ...
                    @if(sizeof($data) > 0)
                        ...
                        @endif
                        <br>
                        <form id="formListItem" method="POST" action="{{FuncLib::getBaseUrl()}}admin/member/delete" class="formListItem" name="txtForm">
                            <table class="table table-bordered table-hover">
                                ...
                                <tr>
                                    ...
                                    <th width="10%">Email đăng nhập</th>
                                    <th width="8%">ID Facebook</th>
                                    <th width="8%">ID google</th>
                                ...
                                @foreach($data as $k=>$item)
                                    <tr>
                                        ...
                                        <td>
                                            <label class="pos-rel">
                                                <input class="ace checkItem" name="checkItem[]" value="{{$item['member_id']}}" type="checkbox">
                                                <span class="lbl"></span>
                                            </label>
                                        </td>
                                        <td>{{$item['member_full_name']}}</td>
                                        <td>{{$item['member_mail']}}</td>
                                        <td>{{$item['member_id_facebook']}}</td>
                                        <td>{{$item['member_id_google']}}</td>
                                        <td>{{$item['member_address']}}</td>
                                        <td>{{date('d/m/Y', $item['member_created'])}}</td>
                                        <td>
                                        ...
                                        <td>
                                            <a href="{{FuncLib::getBaseUrl()}}admin/member/edit/{{$item['member_id']}}" title="Cập nhật">
